...
**いまは 20 項目すべて OK になるのが正常。** SKIP が出たらエンドポイントが消えている。
...
| 壊れた JSON と 413 が**契約の形**で返る | Minimal API に任せると本文の無い 400 になる（`docs/08` §4.1） |
| `Authorization` ヘッダを弾かない | 認証は実装していないが、付けてくるクライアントがいる |
| 長文でヘッダが先に返る | 全部合成してから返すと約 7,000 文字で接続タイムアウトを踏む |
