...
5. **`SpeechSynthesizer` の寿命と同時実行。**
   1 インスタンスを複数のリクエストから同時に叩いていないか。
   使い回すなら排他が要る。リクエストごとに作るなら、必ず破棄しているか。
   `--concurrency` を上げたときに壊れないか。

6. **`SpeakingRate` をクランプしているか。**
   setter は値を検証せず、0.25 も 6.5 も素通りする。有効域は 0.5〜6.0。
   クランプが無いと「設定できたのに速度が変わらない」という、
   API からは正常に見える不具合になる。
