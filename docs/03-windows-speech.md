...
| | |
|---|---|
| 合成（PowerShell から直接） | 19 文字の文（4.3 秒の音声）で **73 ms**。RTF ≒ 0.017 |
| **合成（サーバ経由・実測）** | **2.2 ms/文字**。3,000 文字 → 644 秒の音声を **6.6 秒**で合成（RTF ≒ **0.010**） |
| 発話の長さ | **0.21 秒/文字** 前後（日本語・speed 1.0） |
| 48 kHz へのアップサンプル | 9.8 秒の音声で **71〜81 ms**。合成時間の 3〜4 割を占める |
| GPU | 不要 |
...
ただし **2.2 ms/文字は無限に速いわけではない**。接続タイムアウト 15 秒に対して
**約 7,000 文字**で届く。長文でヘッダを先に返す理由はここ（[02](02-http-contract.md) §4.3）。
