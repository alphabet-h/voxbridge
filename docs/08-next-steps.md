...
> テスト 163 件・契約適合スモーク 21 項目がすべて通ります。
> レビューで見つかった**プロセス即死バグ（§4.0）を含む 5 件は修正済み**です。
> 配布（§1.1）も済み、速さも打ち止めまで詰めました（§3.11 §3.12）。
>
> **次にやること**: §2。**急ぐものはありません。** どれも「実害が出てから」と決めたものです。
...
| 検査 | `scripts\check.ps1`（BOM → build → format → test → 契約照合）/ テスト 163 件 |
| スモーク | `node scripts/smoke.mjs` — 21 項目すべて OK |
| 配布 | **単一 exe を publish して確認済み**（下記 §1.1） |
| **実際の音** | **耳で確認済み**（2026-08-16 に再確認）。3 声とも本文が聞き取れ、ayumi / haruka は女性、ichiro は男性。**別の声を 2 つ挟んでから戻しても同じ声**（§3.12） |
...
### 3.12 合成器は声ごとに使い回す（2026-08-16 に測って決めた）

**「遅いと分かってから」と保留していた項目**です。§3.11 と同じく、先に基準を決めてから
測りました（20 文字の短文で 2 割以上縮むならやる。発注元はセリフ単位の短文が主なので、
比率が最も厳しく出る条件を基準にした）。

結果は **58.7% 短縮**。基準を大きく超えたので実装しました。

| 端から端まで（ヘッダまで） | 前 | 後 |
|---|---|---|
| 20 文字 | 52 ms | **21 ms** |
| 20 文字・3 声を巡回 | 66 ms | **17 ms** |
| 200 文字 | 201 ms | 165 ms |
| 6,000 文字 | 210 ms | 178 ms |

**声ごとに 1 つ持ちます。** 1 つを使い回して `Voice` を差し替える案は測って捨てました
（代入は 0.001 ms なのに、次の合成が 2 倍以上遅くなる）。詳細は
[03](03-windows-speech.md) §7.2。

途中で分かったこと:

- **中断の直後 1 回だけ音が違う**のは、使い回しとは無関係な Windows 側の性質だった
  （[03](03-windows-speech.md) §6.1）。3 方式で 150 件 abort を通しても差が出ない
- 出力は**バイト単位で変わらない**。毎回 `new` していた頃と同じ WAV が返る
- **1,000 文字の入力はほとんど速くならない**（833 → 817 ms）。合成器の生成は
  リクエストごとに 1 回なので、長文では 900 ms の中に 20 ms が埋もれる。
  効くのは短文と、分割が多い長文
