...
**配るのは `voxbridge.exe` だけ。** 同じ場所に出る `.pdb` にはビルドしたマシンの
ソースの絶対パス（ユーザー名を含む）が入ります。exe 側には入っていないことを確認済み。
...
改名は中身を先に、フォルダ名を後にして 2 段階でやりました（作業中にパスが変わると
セッションが壊れるため）。**フォルダ名も同日に `voxbridge` へ移行済みです。**
